...
from elfina.modules.helper_funcs.extraction import extract_user
...
from elfina import dispatcher, SUDO_USERS, SUPPORT_USERS, LOGGER
from elfina.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler

import elfina.modules.helper_funcs.fun_strings as fun
...
`/decide` can be also used with regex like: `elfina? <question>: randomly answer "Yes, No" etc.`

Some other regex filters are:
`me too` | `goodmorning` | `goodnight`.

elfina will reply random strings accordingly when these words are used!
...
DECIDE_HANDLER = DisableAbleMessageHandler(
    Filters.regex(r"(?i)^elfina\?"), decide, friendly="decide"
)
