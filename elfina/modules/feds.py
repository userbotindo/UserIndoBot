...
from elfina.modules.helper_funcs.chat_status import is_user_admin
from elfina.modules.helper_funcs.extraction import (
    extract_user,
    extract_unt_fedban,
    extract_user_fban,
)
from elfina.modules.helper_funcs.string_handling import markdown_parser
from elfina.modules.disable import DisableAbleCommandHandler

import elfina.modules.sql.feds_sql as sql

from elfina.modules.helper_funcs.alternate import (
    send_message,
    typing_action,
    send_action,
)
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "elfina_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="elfina_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "elfina_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="elfina_fbanned_users.csv",
