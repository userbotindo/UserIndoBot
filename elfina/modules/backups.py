...
import elfina.modules.sql.notes_sql as sql
from elfina import dispatcher, LOGGER, OWNER_ID, MESSAGE_DUMP
from elfina.__main__ import DATA_IMPORT
from elfina.modules.helper_funcs.chat_status import user_admin
from elfina.modules.helper_funcs.alternate import typing_action

# from elfina.modules.rules import get_rules
import elfina.modules.sql.rules_sql as rulessql

# from elfina.modules.sql import warns_sql as warnssql
import elfina.modules.sql.blacklist_sql as blacklistsql
from elfina.modules.sql import disable_sql as disabledsql

# from elfina.modules.sql import cust_filters_sql as filtersql
# import elfina.modules.sql.welcome_sql as welcsql
import elfina.modules.sql.locks_sql as locksql
from elfina.modules.connection import connected
...
def export_data(update, context):
    ...
    f = open("elfina{}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("elfina{}.backup".format(chat_id), "rb"),
        caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `ElfinaFara-Backup` is specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("elfina{}.backup".format(chat_id))  # Cleaning file
